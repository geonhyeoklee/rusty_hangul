...
impl Nfd {
  ...
  pub fn normalize_from_u32(letter_code: u32) -> Option<Self> {
    ...
    let choseong_index = hangul_code / (JUNGSEONG_COUNT + JONGSEONG_COUNT);
    let jungseong_index = (hangul_code % (JUNGSEONG_COUNT + JONGSEONG_COUNT)) / JONGSEONG_COUNT;
...
fn test_normalize_from_u32() {
  ...
  println!("{:?}", choseong_code); // 0x1105
  println!("{:?}", jungseong_code); // 4469
  println!("{:?}", jongseong_code); // 4527
}
